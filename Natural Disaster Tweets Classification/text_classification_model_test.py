...
x_train, y_train, train_le, train_labels, word_index, tokenizer = LoadDataset.read_train_data(data_train_path, 20000, MAX_SEQUENCE_LENGTH, "\t")
x_dev, y_dev, dev_le, dev_labels, _,id_list = LoadDataset.read_dev_data(data_val_path, tokenizer, MAX_SEQUENCE_LENGTH, "\t", train_le)
x_test, y_test, test_le, test_labels, _,id_list = LoadDataset.read_dev_data(data_test_path, tokenizer, MAX_SEQUENCE_LENGTH, "\t", train_le)
...
embedding_matrix = LoadDataset.prepare_embedding(word_index, emb_model, 20000, 300)
...
callback = callbacks.EarlyStopping(monitor='val_acc', patience=100, verbose=0, mode='max')
...
result_val = model.predict([x_dev], batch_size=8, verbose=1)
...
AUC, accu, P, R, F1, report = metrics.calculate_metrics(y_dev, result_val, train_le)
result = str("{0:.4f}".format(accu)) + "\t" + str("{0:.4f}".format(P)) + "\t" + str(
    "{0:.4f}".format(R)) + "\t" + str("{0:.4f}".format(F1))+ "\t" + str("{0:.4f}".format(AUC)) + "\n"
print(result)
print (report)
out_file.write(data_val_path + "\n")
out_file.write(result)
out_file.write(report)

###### Test
test_out_label_file_name = dir_name + "\\" + base_name + "_test_labels.txt"
AUC, accu, P, R, F1, report = metrics.calculate_metrics(y_test, result_test, train_le)
result = str("{0:.4f}".format(accu)) + "\t" + str("{0:.4f}".format(P)) + "\t" + str(
    "{0:.2f}".format(R)) + "\t" + str("{0:.4f}".format(F1)) + "\t" + str("{0:.4f}".format(AUC))+ "\n"
print("results-cnn:\t"+base_name+"\t"+result)
print (report)
out_file.write(data_test_path+ "\n")
out_file.write(result)
out_file.write(report)

conf_mat_str = metrics.format_conf_mat(y_test, result_test, train_le)
