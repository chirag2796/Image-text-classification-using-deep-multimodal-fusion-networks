...
#the last layer here is flatten, we take the output of flatten as activations of input images and use these as input to train the combination model
model_cnn.add(layers.Flatten(name='Flatten'))

# Loading model weights so we generate activations from trained baseline image classifier
model_cnn.load_weights('./inception_model_weights_0.72.hdf5')

# In[30]:

# freezing all layers so this part is not trained when the fusion ANN is trained
...
#the last layer here is dense, we take the output of dense as activations of input images and use these as input to train the combination model. Notice we have remove the last layer of type 1 model and only kept teh second last dense layer
model_lstm = models.Model([input_ids, input_masks, input_segments], pred)

# Loading model weights so we generate activations from trained baseline text classifier
model_lstm.load_weights('./bert_rnn_model_weights_20epochs.hdf5')
# In[ ]:
# freezing all layers so this part is not trained when the fusion ANN is trained
...
#combining the two models to train together
